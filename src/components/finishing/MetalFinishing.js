...
function MetalFinishing() {
  ...
  const handleSubmit = () => {
    ...
    console.log('Metal Finishing Data Submitted:', { 
      inputQuantity, 
      outputQuantity, 
    ...
    setScrapQuantity('');
    setError('');
    setSuccess('Metal Finishing data submitted successfully!');
  };

  return (
    <Container maxWidth="lg" sx={enhancedStyles.container}>
      <Typography variant="h4" component="h1" gutterBottom>
        Metal Finishing (MF) Management
      </Typography>
      
      <Paper sx={enhancedStyles.paperSection}>
        <Typography variant="h6" gutterBottom>
          Input Metal Finishing (MF) Data
        </Typography>
        <Grid container spacing={3}>
          <Grid item xs={12} md={4}>
            <TextField
              fullWidth
              label="Input Quantity (to Metal Finishing)"
              type="number"
              value={inputQuantity}
           ...
           <Grid item xs={12} md={4}>
            <TextField
              fullWidth
              label="Output Quantity (from Metal Finishing)"
              type="number"
          ...
          <Grid item xs={12}>
            <Button variant="contained" color="primary" onClick={handleSubmit}>
              Submit Metal Finishing (MF) Data
            </Button>
          </Grid>
        </Grid>
      </Paper>
